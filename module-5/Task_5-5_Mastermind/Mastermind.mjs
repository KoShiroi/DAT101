...
import { TColorPicker, playerAnswer } from "./ColorPicer.js";
...
function drawGame() {
  ...
  spcvs.clearCanvas();
  menu.drawBackground();
  computerAnswer.forEach(element =>{
    element.draw()
  })
  menu.draw();
...
function createColorPicker(){
  let colorIndex=0
  let aIndex=colorPicker.length;
  Object.keys(MastermindBoard.ColorPicker).forEach(element => {
    colorPicker.push(new TColorPicker(MastermindBoard.ColorPicker[element]))
    colorPicker[colorIndex+aIndex].index=colorIndex;
    colorIndex++;
  });
}
function createComputerAnswer(){
  for(let i=0;i<4;i++){
    const rng=Math.floor(Math.random()*SpriteInfoList.ColorPicker.count);
    const spColor=new TSprite(spcvs,SpriteInfoList.ColorPicker,MastermindBoard.ComputerAnswer[i].x,MastermindBoard.ComputerAnswer[i].y);
    spColor.index=rng;
